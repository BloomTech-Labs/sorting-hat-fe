...
function Quiz(props) {
  ...
  const answerButton = (answer, index) => (
    <div
      key={`div-${index}`}
      className="flex items-center justify-start w-full mt-1 fira-sans mb-2"
    >
  ...
  return (
    <div className="w-1/2 h-screen p-1 m-auto mt-3">
      ...
      <h1 className="pt-4 mt-2 text-xl text-4xl fira-sans">
        Question {curQuesIndex + 1}
      </h1>

      <ProgressBar progress={questions[curQuesIndex].id / totalNumQues} />
      {/*Question Text*/}
      <h1 className="mt-2 protoGray fira-sans text-2xl">
        {questions[curQuesIndex].question}
      </h1>
      {/*Answers to the Questions*/}
      <div className="flex flex-col w-full mt-4 mb-2 text-lg">
        {answers.map(
          (answer, index) =>
