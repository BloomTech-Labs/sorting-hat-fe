...
function Quiz(props) {
	...
	const answerButton = (answer, index) => (
		<div key={`div-${index}`} className="ansBtn flex items-center justify-start w-full mt-1 fira-sans mb-5">
		...
		<div className="mr-8 ml-8">
			<div className="flex flex-col justify-center items-center md:max-h-screen md:h-screen mt-20 md:m-auto text-sm lg:text-2xl">
				...
				<div className="lg:w-1/2 p-2 mt-10 w-full">
					...
					{/*Question Text*/}
					<h1 className="mt-2 text-black lg:text-gray-700 fira-sans">{questions[curQuesIndex].question}</h1>
					...
					<div className="flex flex-col w-full mt-4 mb-2 py-5 text-sm lg:text-lg">
						{answers.map(
							(answer, index) =>
								answer.question_id === questions[curQuesIndex].id && answerButton(answer, index)
						)}
					</div>
